...
class UserSubscription(models.Model):
    ...
    def billing_cycle_anchor(self):
        ...
        optional delay for new subscription in Stripe checkout
        """
    ...
    def save(self, *args, **kwargs):
        if (self.original_period_start is None and self.current_period_start is not None):
            self.original_period_start = self.original_period_start
        super().save(*args, **kwargs)
...
def user_sub_post_save(sender, instance, *args, **kwargs):
    ...
    if not ALLOW_CUSTOM_GROUPS:
        ...
        # group_ids = groups.values_lists('id', flat=True) # 1, 2, 3
